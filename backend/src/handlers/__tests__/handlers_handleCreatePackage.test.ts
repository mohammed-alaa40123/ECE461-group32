...
describe('handleCreatePackage', () => {
  ...
  it('should return 401 if user does not have upload permission', async () => {
    ...
    expect(mockedSendResponse).toHaveBeenCalledWith(401, { message: 'You do not have permission to upload packages.' });
  });
  ...
  it('should return 200 if package creation is successful with URL', async () => {
    ...
    expect(result.statusCode).toBe(500);
    expect(mockedSendResponse).toHaveBeenCalledWith(500, expect.any(Object));
  });
