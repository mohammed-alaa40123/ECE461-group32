...
describe('Access Handlers', () => {
  ...
  describe('handleDeleteGroup', () => {
    ...
    it('should retrieve user groups and permissions as admin', async () => {
      ...
      expect(response).toEqual({
        statusCode: 200,
        body: JSON.stringify({
          userGroups: [{ id: 1, name: 'Admins' }],
          userPermissions: [{ id: 101, name: 'Read' }],
        }),
        headers: { 'Content-Type': 'application/json' },
      });
    });
    ...
    it('should return 403 if user is not admin', async () => {
      ...
      expect(mockedSendResponse).toHaveBeenCalledWith(403, { message: 'Admin privileges required to retrieve user groups and permissions.' });
      expect(response).toEqual({
        statusCode: 403,
  ...
  describe('handleDeletePermission', () => {
    ...
    it('should return 403 if user is not admin', async () => {
      ...
      expect(mockedAuthenticate).toHaveBeenCalledWith(requestHeaders);
      expect(mockedPool).not.toHaveBeenCalled();
      expect(mockedSendResponse).toHaveBeenCalledWith(403, {
