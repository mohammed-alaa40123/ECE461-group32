...
describe('Handlers', () => {
  ...
  describe('handleResetRegistry', () => {
    it('should reset the registry successfully', async () => {
      ...
      expect(mockedPoolQuery).toHaveBeenCalledTimes(11); // BEGIN, TRUNCATE x7, COMMIT
      expect(response).toEqual({
        statusCode: 200,
        body: JSON.stringify({
    ...
    it('should return 403 if user is not admin', async () => {
      ...
      expect(response).toEqual({
        statusCode: 200,
        body: JSON.stringify({ message: 'Registry is reset, only the default admin user is retained.' }),
        headers: { 'Content-Type': 'application/json' },
      });
    });
  ...
  describe('handleSearchPackagesByRegEx', () => {
    ...
    it('should return 400 if RegEx is missing', async () => {
      ...
      expect(response).toEqual({
        statusCode: 400,
        body: JSON.stringify({ message: 'There is a missing field in the PackageRegEx or it is formed improperly, or is invalid.' }),
        headers: { 'Content-Type': 'application/json' },
      });
    });
  ...
  describe('handleGetTracks', () => {
    it('should return planned tracks successfully', async () => {
      ...
      expect(response.statusCode).toBe(200);
      const body = JSON.parse(response.body);
      expect(body).toEqual({ plannedTracks: ['Access control track'] });
    });
    ...
    it('should handle authentication errors', async () => {
      ...
      expect(response).toEqual({
        statusCode: 200,
        body: JSON.stringify({ plannedTracks:['Access control track']}),
        headers: { 'Content-Type': 'application/json' },
      });
    });
